...
export function SyncMonitor({ onManualSync, className }: SyncMonitorProps) {
  ...
  const loadStats = async () => {
    try {
      // Synchronisation désactivée temporairement
      const syncStats = {
        lastSync: null,
        pendingChanges: 0,
  ...
  const handleSyncRequest = async (tag: string, isLastChance: boolean) => {
    try {
      ...
      toast({
        ...
        description:
          "La synchronisation manuelle est temporairement désactivée.",
      });
  ...
  const handleReregisterRequest = async (
    originalTag: string,
    error: string
  ) => {
    console.log(
      `🔄 Client: Ré-enregistrement requis pour ${originalTag}: ${error}`
    );
    ...
    try {
      ...
      if (onManualSync) {
        ...
        toast({
          ...
          description:
            "La synchronisation automatique est temporairement désactivée.",
        });
      }
  ...
  const hasOldPendingData =
    stats.oldestPendingDate &&
    Date.now() - new Date(stats.oldestPendingDate).getTime() >
      24 * 60 * 60 * 1000;

  const hasRecentSuccess =
    stats.lastSuccessDate &&
    Date.now() - new Date(stats.lastSuccessDate).getTime() <
      24 * 60 * 60 * 1000;
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <Badge
                variant={stats.periodicSyncSupported ? "default" : "secondary"}
                className="ml-2"
              >
            ...
            <div>
              ...
              <span className="ml-2">
                {stats.lastSyncCheck
                  ? new Date(stats.lastSyncCheck).toLocaleTimeString()
                  : "Jamais"}
              </span>
            </div>
          </div>
