...
import { handlePrint, handlePrintDirect, handlePrintBothBonAndInvoice, handlePrintDirectBoth } from "@/utils/peseeUtils";
...
export default function PeseeSpace({
  ...
  const [printTitle, setPrintTitle] = useState<string>("Aperçu avant impression");
  ...
  const isClientAddressComplete = (client: Client): boolean => {
    return Boolean(client.adresse && client.adresse.trim() !== "" && client.codePostal && client.codePostal.trim() !== "" && client.ville && client.ville.trim() !== "");
  };

  // Fonction wrapper pour effacer les erreurs de validation
  const updateCurrentTabWithValidation = (updates: Partial<PeseeTab["formData"]>) => {
  ...
  const loadPeseeForEdit = useCallback(async (peseeId: number) => {
    ...
    if (isEditingInProgressRef.current || editingTabId) {
      console.log("[loadPeseeForEdit] Une édition est déjà en cours, on ignore cette demande");
      return;
    }
    ...
    try {
      ...
      const isExported = pesee.exportedAt && pesee.exportedAt.length > 0 || pesee.numeroBonExported || pesee.numeroFactureExported;
      if (isExported) {
        toast({
          title: "Modification impossible",
          description: "Cette pesée a déjà été exportée et ne peut plus être modifiée.",
          variant: "destructive"
        });
        onEditHandled?.();
        return;
      ...
      const formDataForEdit = {
        ...
        moyenPaiement: pesee.moyenPaiement as "ESP" | "CB" | "CHQ" | "VIR" | "PRVT" || "ESP",
        typeClient: pesee.typeClient || "professionnel",
        clientId: pesee.clientId || 0
      };
      const newTabId = createEditTab(formDataForEdit, pesee.nomEntreprise || "");
      if (!newTabId) {
        throw new Error("Impossible de créer un nouvel onglet pour l'édition");
      }
  ...
  }, [toast, onEditHandled, editingTabId, createEditTab, setEditingPeseeId, setEditingTabId, setActiveTabId]);
    ...
    const initializeDefaultProduct = async () => {
      if (tabs.length > 0 && products.length > 0) {
        ...
        if (currentTab && currentTab.formData.produitId === 0) {
          ...
          const dechetsVerts = products.find(p => p.nom.toUpperCase() === "DECHETS VERT");
  ...
  const generateNextClientCode = async (): Promise<string> => {
    try {
      ...
      const clientsWithCode = await db.clients.where("codeClient").above("").toArray();
      ...
      const numericCodes = clientsWithCode.map(client => {
  ...
  const handleAddChantier = async () => {
    ...
    try {
      ...
      if (client) {
        ...
        const updatedChantiers = [...(fullClient.chantiers || []), newChantier.trim()];
  ...
  const handleAddPlaque = async () => {
    ...
    try {
      ...
      if (client) {
        ...
        const updatedPlaques = [...(fullClient.plaques || []), newPlaque.trim()];
  ...
  const handleUpdateClient = async () => {
    try {
      ...
      const clientData = {
        ...
        transporteurId: newClientForm.transporteurId ?? fullClient.transporteurId ?? 0,
        // Préserver les tarifs préférentiels existants si non définis dans le formulaire
        tarifsPreferentiels: newClientForm.tarifsPreferentiels ?? fullClient.tarifsPreferentiels ?? {},
        id: newClientForm.id,
        updatedAt: new Date()
      } as Client;
  ...
  const validateNewClient = (): boolean => {
    ...
    if (!newClientForm.codeClient || typeof newClientForm.codeClient !== "string" || newClientForm.codeClient.trim() === "") {
      return false;
    }
    if (newClientForm.typeClient === "particulier") {
  ...
  const handleSaveOnly = async (typeDocument: "bon_livraison" | "facture" | "les_deux" = "bon_livraison") => {
  ...
  const handleSaveAndPrint = async (typeDocument: "bon_livraison" | "facture" | "les_deux" = "bon_livraison") => {
    ...
    if (result && result.savedPeseeId) {
      ...
      try {
        ...
        if (savedPesee) {
          ...
          const client = savedPesee.clientId ? clients.find(c => c.id === savedPesee.clientId) || null : null;
          ...
          const formDataForPrint: PeseeTab["formData"] = {
            ...
            moyenPaiement: savedPesee.moyenPaiement as "ESP" | "CB" | "CHQ" | "VIR" | "PRVT",
          ...
          console.log(`[handleSaveAndPrint] Appel handlePrintDirect avec isInvoice=${isInvoice}...`);
          await handlePrintDirect(formDataForPrint, products, transporteurs, isInvoice, client);
          ...
          console.error("[handleSaveAndPrint] ERREUR: savedPesee est null/undefined");
          toast({
            title: "Erreur",
        ...
        console.error("[handleSaveAndPrint] ERREUR lors de la récupération/impression:", error);
        toast({
          title: "Erreur",
      ...
      console.warn("[handleSaveAndPrint] ATTENTION: result est null, la pesée n'a pas été sauvegardée");
    }
    setIsSaveDialogOpen(false);
  };
  const handleSaveAndPrintInvoice = async (typeDocument: "bon_livraison" | "facture" | "les_deux" = "facture") => {
    console.log("[handleSaveAndPrintInvoice] Début, typeDocument:", typeDocument);
    // Capturer l'ID de l'onglet actif AVANT la sauvegarde
    const currentTabId = activeTabId;
    // Vérifier si c'était le dernier onglet avant la sauvegarde
    const wasLastTab = tabs.length === 1;
    ...
    if (result && result.savedPeseeId) {
      ...
      try {
        // Récupérer la pesée sauvegardée depuis la DB
        const savedPesee = await db.pesees.get(savedPeseeId);
        console.log("[handleSaveAndPrintInvoice] savedPesee récupérée:", savedPesee);
        if (savedPesee) {
          // Récupérer le client
          const client = savedPesee.clientId ? clients.find(c => c.id === savedPesee.clientId) || null : null;
          // Créer formDataForPrint à partir de la pesée sauvegardée
          ...
          await handlePrintDirect(formDataForPrint, products, transporteurs, true, client);
          console.log("[handleSaveAndPrintInvoice] Fenêtre d'impression ouverte");
          ...
          console.error("[handleSaveAndPrintInvoice] ERREUR: savedPesee est null/undefined");
          toast({
            title: "Erreur",
            description: "Impossible de récupérer la pesée sauvegardée.",
            variant: "destructive"
        ...
        console.error("[handleSaveAndPrintInvoice] ERREUR lors de la récupération/impression:", error);
        toast({
          title: "Erreur",
          description: "Une erreur est survenue lors de l'impression.",
          variant: "destructive"
      ...
      console.warn("[handleSaveAndPrintInvoice] ATTENTION: result est null, la pesée n'a pas été sauvegardée");
    }
    setIsSaveDialogOpen(false);
  };
  const handleSavePrintBonAndInvoice = async (typeDocument: "bon_livraison" | "facture" | "les_deux" = "les_deux") => {
    console.log("[handleSavePrintBonAndInvoice] Début, typeDocument:", typeDocument);
    // Capturer l'ID de l'onglet actif AVANT la sauvegarde
    const currentTabId = activeTabId;
    // Vérifier si c'était le dernier onglet avant la sauvegarde
    const wasLastTab = tabs.length === 1;
    ...
    if (result && result.savedPeseeId) {
      ...
      try {
        // Récupérer la pesée sauvegardée depuis la DB
        const savedPesee = await db.pesees.get(savedPeseeId);
        console.log("[handleSavePrintBonAndInvoice] savedPesee récupérée:", savedPesee);
        if (savedPesee) {
          // Récupérer le client
          const client = savedPesee.clientId ? clients.find(c => c.id === savedPesee.clientId) || null : null;
          // Créer formDataForPrint à partir de la pesée sauvegardée
          ...
          // Ouvrir directement la fenêtre d'impression du navigateur
          console.log("[handleSavePrintBonAndInvoice] Appel handlePrintDirectBoth...");
          await handlePrintDirectBoth(formDataForPrint, products, transporteurs, client);
          console.log("[handleSavePrintBonAndInvoice] Fenêtre d'impression ouverte");
          ...
          console.error("[handleSavePrintBonAndInvoice] ERREUR: savedPesee est null/undefined");
          toast({
            title: "Erreur",
            description: "Impossible de récupérer la pesée sauvegardée.",
            variant: "destructive"
        ...
        console.error("[handleSavePrintBonAndInvoice] ERREUR lors de la récupération/impression:", error);
        toast({
          title: "Erreur",
          description: "Une erreur est survenue lors de l'impression.",
          variant: "destructive"
      ...
      console.warn("[handleSavePrintBonAndInvoice] ATTENTION: result est null, la pesée n'a pas été sauvegardée");
    }
    setIsSaveDialogOpen(false);
  };
  const savePesee = async (typeDocument: "bon_livraison" | "facture" | "les_deux" = "bon_livraison"): Promise<{
    savedPeseeId: number;
    ...
    try {
      ...
      const chantierToUse = currentData?.chantierLibre?.trim() || currentData?.chantier?.trim() || "";
        ...
        if (currentData.clientId) {
          ...
          if (client && isClientAddressComplete(client)) {
            ...
            toast({
              ...
              description: "Chantier suggéré à partir de l'adresse principale du client. Vous pouvez le remplacer si nécessaire.",
              variant: "default"
            });
            ...
            const client = clients.find(c => c.id === currentData.clientId);
            const hasPartialAddress = client && (client.adresse || client.codePostal || client.ville);
            toast({
              title: "Chantier obligatoire",
              description: hasPartialAddress ? "Impossible de valider la pesée : aucun chantier sélectionné et adresse client incomplète. Complétez l'adresse du client ou sélectionnez un chantier existant." : "Impossible de valider la pesée : aucun chantier sélectionné et aucune adresse client disponible. Ajoutez une adresse au client ou sélectionnez un chantier existant.",
              variant: "destructive"
            });
            return null;
          }
      ...
      try {
        ...
        const taxesHT = activeTaxes.reduce((sum, tax) => sum + totalHT * (tax.taux / 100), 0);
        const tauxTVAProduit = selectedProduct.tauxTVA || 20;
        const montantTVAProduit = totalHT * (tauxTVAProduit / 100);
        const montantTVATaxes = activeTaxes.reduce((sum, tax) => sum + totalHT * (tax.taux / 100) * ((tax.tauxTVA ?? 20) / 100), 0);
      ...
      if (isEditing) {
        ...
        // Vérifier si la pesée a déjà été exportée
        const isExported = existingPesee.exportedAt && existingPesee.exportedAt.length > 0 || existingPesee.numeroBonExported || existingPesee.numeroFactureExported;
        if (isExported) {
          toast({
            title: "Modification impossible",
            description: "Cette pesée a déjà été exportée et ne peut plus être modifiée.",
      ...
      if (isEditing && existingPesee) {
        ...
        if (existingPesee.numeroFacture && !existingPesee.numeroFactureExported) {
          ...
          if (!numeroBon) {
            ...
            if (numeroFacture && numeroFacture.startsWith("FA")) {
              ...
              if (!isNaN(seqNum)) {
                ...
                const blExists = existingPesee?.id ? peseesWithBL.some(p => p.id !== existingPesee.id) : peseesWithBL.length > 0;
                if (blExists) {
                  ...
                  while (true) {
                    const blExists2 = (await db.pesees.where("numeroBon").equals(newBL).toArray()).some(p => existingPesee?.id ? p.id !== existingPesee.id : true);
                    const faExists = (await db.pesees.where("numeroFacture").equals(newFA).toArray()).some(p => existingPesee?.id ? p.id !== existingPesee.id : true);
          ...
          if (!numeroFacture) {
            ...
            if (numeroBon && numeroBon.startsWith("BL")) {
              ...
              if (!isNaN(seqNum)) {
                ...
                const faExists = existingPesee?.id ? peseesWithFA.some(p => p.id !== existingPesee.id) : peseesWithFA.length > 0;
                if (faExists) {
                  ...
                  // Vérifier que les nouveaux numéros sont disponibles
                  while (true) {
                    const blExists = (await db.pesees.where("numeroBon").equals(newBL).toArray()).some(p => existingPesee?.id ? p.id !== existingPesee.id : true);
                    const faExists2 = (await db.pesees.where("numeroFacture").equals(newFA).toArray()).some(p => existingPesee?.id ? p.id !== existingPesee.id : true);
        ...
        if (typeDocument === "bon_livraison" || typeDocument === "les_deux") {
          if (typeDocument === "les_deux") {
            ...
            let blExists = (await db.pesees.where("numeroBon").equals(numeroBon).count()) > 0;
            let faExists = (await db.pesees.where("numeroFacture").equals(numeroFacture).count()) > 0;
            while (blExists || faExists) {
              nextSeqNum++;
              numeroBon = `BL${nextSeqNum}`;
              numeroFacture = `FA${nextSeqNum}`;
              blExists = (await db.pesees.where("numeroBon").equals(numeroBon).count()) > 0;
              faExists = (await db.pesees.where("numeroFacture").equals(numeroFacture).count()) > 0;
            }
          } else {
      ...
      const peseeData: Partial<Pesee> = {
        ...currentData,
        numeroBon: numeroBon || "",
        // Vide si seulement facture
        numeroFacture: numeroFacture || undefined,
        typeDocument,
        dateHeure: isEditing && existingPesee ? existingPesee.dateHeure : new Date(),
        poidsEntree,
        // déjà en tonnes
        poidsSortie,
        // déjà en tonnes
        ...
        prixHT: totalHT,
        prixTTC: totalTTC,
        chantier: chantierToUse,
        // Utiliser chantierLibre en priorité, sinon chantier
        chantierLibre: currentData.chantierLibre || undefined,
        // Sauvegarder le chantier libre
        transporteurId: currentData.transporteurId || undefined,
        transporteurLibre: currentData.transporteurLibre || undefined,
        // Sauvegarder le transporteur libre
        typeClient: currentData.typeClient || "professionnel",
        synchronized: false,
        updatedAt: new Date()
      };
   ...
   */
  const checkAndGenerateTrackDechet = async (savedPeseeId: number, peseeData: Pesee) => {
    try {
      // Trouver le produit
      ...
      const [client, transporteur] = await Promise.all([peseeData.clientId ? db.clients.get(peseeData.clientId) : Promise.resolve(null), peseeData.transporteurId ? db.transporteurs.get(peseeData.transporteurId) : Promise.resolve(null)]);
      if (!client) {
      ...
      if (isApplicable) {
        ...
        await trackDechetProcessor.addPeseeToQueue(savedPeseeId, client.id!, transporteur?.id || null, product.id!, product.codeDechets);
        toast({
          title: "📋 Track Déchet",
  ...
  const handleCloseTab = (tabId: string) => {
    ...
    const hasData = tab.formData.clientId > 0 || tab.formData.produitId > 0 || tab.formData.poidsEntree || tab.formData.poidsSortie || tab.formData.plaque || tab.formData.nomEntreprise;
    if (hasData || isEditingTab) {
      // Afficher une confirmation
      const message = isEditingTab ? "Cet onglet contient une pesée en cours de modification. Êtes-vous sûr de vouloir le fermer ?" : "Cet onglet contient des données non sauvegardées. Êtes-vous sûr de vouloir le fermer ?";
  ...
  return <div className="space-y-4 pt-14">
        ...
        <div className="px-6 py-2 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0 overflow-hidden">
              <EnhancedTabs tabs={tabs.map(tab => ({
              id: tab.id,
              label: getTabLabel(tab.id),
              onClose: () => handleCloseTab(tab.id),
              closeable: tabs.length > 1,
              isEditing: tab.isEditing || false
            }))} activeTabId={showRecentTab ? null : activeTabId} onTabSelect={tabId => {
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Button variant={showRecentTab ? "default" : "outline"} onClick={async () => {
              ...
              setShowRecentTab(false);
              handleCreateNewTab();
            }} className="h-9 px-4 border-white bg-green-600 hover:bg-green-700 hover:text-white text-white">
                + Nouveau
              </Button>
            </div>
          </div>
      ...
      <Tabs value={showRecentTab ? "recentes" : activeTabId} onValueChange={async value => {
      if (value === "recentes") {
        await loadData(); // Attendre le rafraîchissement avant d'afficher
        setShowRecentTab(true);
      } else {
              ...
              <CardContent className="p-3">
                <div className="grid grid-cols-1 lg:grid-cols-[50%_50%] items-stretch">
                  ...
                  <div className="space-y-2 lg:border-r lg:border-gray-200 lg:pr-4 flex flex-col h-full">
                    <PeseeFormSection currentData={tab.formData} clients={clients} products={products} transporteurs={transporteurs} updateCurrentTab={updateCurrentTabWithValidation} onAddClient={prepareNewClientForm} isAddClientDialogOpen={isAddClientDialogOpen} setIsAddClientDialogOpen={setIsAddClientDialogOpen} newClientForm={newClientForm} setNewClientForm={setNewClientForm} handleAddNewClient={handleAddNewClient} handleUpdateClient={handleUpdateClient} validateNewClient={validateNewClient} isAddChantierDialogOpen={isAddChantierDialogOpen} setIsAddChantierDialogOpen={setIsAddChantierDialogOpen} newChantier={newChantier} setNewChantier={setNewChantier} handleAddChantier={handleAddChantier} isAddPlaqueDialogOpen={isAddPlaqueDialogOpen} setIsAddPlaqueDialogOpen={setIsAddPlaqueDialogOpen} newPlaque={newPlaque} setNewPlaque={setNewPlaque} handleAddPlaque={handleAddPlaque} isAddTransporteurDialogOpen={isAddTransporteurDialogOpen} setIsAddTransporteurDialogOpen={setIsAddTransporteurDialogOpen} newTransporteurForm={newTransporteurForm} setNewTransporteurForm={setNewTransporteurForm} handleAddNewTransporteur={handleAddNewTransporteur} validateNewTransporteur={validateNewTransporteur} validationErrors={validationErrors} />
                  </div>

                  {/* Colonne droite : Champs critiques + Actions */}
                  <div className="space-y-2 lg:pl-4 flex flex-col h-full">
                    <ProductWeightSection currentData={tab.formData} products={products} updateCurrentTab={updateCurrentTabWithValidation} validationErrors={validationErrors} />

                    <div className="flex justify-center space-x-3 pt-2 mt-auto">
                      <Button variant="outline" onClick={async () => {
                    if (tab.formData) {
                      const content = await handlePrint(tab.formData, products, transporteurs, false);
        ...
        <TabsContent value="recentes">
          <RecentPeseesTab key={pesees.length > 0 ? `${pesees.length}-${pesees[0]?.id || Date.now()}` : Date.now()} pesees={pesees} products={products} transporteurs={transporteurs} />
        </TabsContent>
      </Tabs>

      <SaveConfirmDialog isOpen={isSaveDialogOpen} onClose={() => setIsSaveDialogOpen(false)} onConfirm={handleSaveOnly} onConfirmAndPrint={handleSaveAndPrint} onConfirmPrintAndInvoice={handleSavePrintBonAndInvoice} moyenPaiement={currentData?.moyenPaiement || "ESP"} />

      {/* Print Preview Dialog */}
      <PrintPreviewDialog isOpen={printPreviewOpen} onClose={() => setPrintPreviewOpen(false)} content={printContent} title={printTitle} />
    </div>;
}