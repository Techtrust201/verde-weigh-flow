...
import {
  validateTrackDechetTokenDetailed,
  ValidationResult,
} from "@/utils/trackdechetApi";
import {
  getGlobalSettings,
  updateTrackDechetSettings,
  GlobalSettings,
...
export default function TrackDechetGlobalSettings() {
  ...
  const [validationTimeoutId, setValidationTimeoutId] =
    ...
    async (token: string) => {
      ...
      try {
        ...
        if (
          ...
          result.errorMessage?.includes("CORS")
        ) {
          toast({
            title: "Token accepté",
            description:
              "Validation limitée par CORS, mais token sauvegardé pour usage hors ligne",
            variant: "default",
          });
        }
      } catch (error) {
  ...
  const handleTrackDechetToggle = async (enabled: boolean) => {
    // Track Déchet est maintenant toujours activé globalement
    // Le déclenchement se fait uniquement au niveau produit lors de la pesée
    toast({
      title: "Information",
      description:
        "Track Déchet est maintenant activé automatiquement pour les produits configurés lors des pesées",
    });
  };
    ...
    if (validationResult) {
      if (validationResult.isValid) {
        ...
        if (
          ...
          validationResult.errorMessage?.includes("CORS")
        ) {
          return {
        ...
        return {
          ...
          message: `Token validé pour ${
            validationResult.userInfo?.name || "utilisateur inconnu"
          }`,
    ...
    <Card className="border-primary/20">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Configuration globale pour générer automatiquement des bordereaux de
          suivi des déchets (BSD) lors des pesées de produits configurés
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm">
              {settings.trackDechetSandboxMode
                ? "Mode sandbox (test)"
                : "Mode production"}
            </span>
            <Badge
              variant={
                settings.trackDechetSandboxMode ? "secondary" : "default"
              }
              className="text-xs"
            >
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-3">
            ...
            <div className="text-xs text-red-700 space-y-1">
              {validationResult.errorType === "format" && (
                <p>
                  Vérifiez que vous avez copié le token complet depuis Track
                  Déchet.
                </p>
              )}
              ...
              {validationResult.errorType === "permissions" && (
                <p>
                  Ce token n'a pas les permissions nécessaires. Contactez votre
                  administrateur Track Déchet.
                </p>
              )}
              {validationResult.errorType === "network" && (
                <p>
        ...
        <div className="bg-muted/50 p-3 rounded-lg space-y-2">
          ...
          >
            ...
            >
              ...
              {settings.trackDechetSandboxMode && "(Sandbox)"}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          ...
          <p className="text-xs text-blue-700">
            Ce token sera utilisé pour tous les produits configurés avec Track
            Déchet lors des pesées. Vous n'avez besoin de le configurer qu'une
            seule fois.
          </p>
        </div>
      </CardContent>
